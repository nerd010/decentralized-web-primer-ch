# Summary

## Introduction
* [Introduction](README.md)

## 第 1 章：安装和初始化 IPFS
* [简介](part1/README.md)
* [下载和安装 IPFS](part1/1-1.md)
* [初始化你的 IPFS 仓库](part1/1-2.md)

## 第 2 章: IPFS 中的文件
* [简介](part2/README.md)
* [上传文件到 IPFS 并检索它](part2/2-1.md)
* [打包有关内容的文件名与目录信息](part2/2-2.md)
* [Pinning -- 告诉 IPFS 保留文件](part2/2-3.md)

## 第 3 章：网络在线 -- 加入分布式 Web
* [简介](part3/README.md)
* [在 IPFS 网络上发布您的节点](part3/3-1.md)
* [在网络上寻找 Peers](part3/3-2.md)
* [从一个 Peer(节点) 中检索内容](part3/3-3.md)

## 第 4 章：与经典（HTTP）网络交互
* [简介](part3/README.md)
* [使用 HTTP 浏览器从本地 IPFS 网关检索文件](part4/4-1.md)
* [通过公共的 ipfs.io 网关获取文件内容](part4/4-2.md)
* [通过任意 IPFS 网关访问 IPFS 的内容](part4/4-3.md)
* [TODO 将 DNS 映射到 IPNS](part4/4-4.md)
* [TODO 在 IPFS 上的流媒体视频](part4/4-5.md)

## 第 5 章：访问和分发 IPFS 的多种方式
* [简介](part5/README.md)
* [内容寻址的能力](part5/5-1.md)
* [从一个 Peer(节点) 中检索内容](part5/5-2.md)
* [阅读关于与经典(HTTP) 网络交互的教程](part5/5-3.md)
    * [使用 HTTP 浏览器从本地 IPFS 网关检索文件](part5/5-3-1.md)
    * [在 ipfs.io 上使用公共 IPFS 网关](part5/5-3-2.md)
    * [通过任意 IPFS 网关访问 IPFS 的内容](part5/5-3-3.md)
* [通过 Tor 网关访问 IPFS 内容（实验性）](part5/5-4.md)
* [通过 Tor transport 运行 IPFS](part5/5-5.md)
* [通过浏览器插件访问 IPFS 内容](part5/5-6.md)
* [Sneakernets -- 移动 USB 驱动器和其它硬件上的数据](part5/5-7.md)

## 第 6 章：在 Permanent Web 上发布更改
* [简介](part6/README.md)
* [在 IPFS节点上设置 IPNS](part6/6-1.md)
* [创建一个指向你的文件的 IPNS 条目](part6/6-2.md)
* [修改你的文件并将修改后的版本添加到  IPFS](part6/6-3.md)
* [更新 IPNS 条目以指向新和版本](part6/6-4.md)

## 第 7 章：分布式 Web 上的隐私和访问权限控制
* [简介](part7/README.md)
* [读者隐私和写作者的隐私](part7/7-1.md)
* [专用(private)网络](part7/7-2.md)
* [加密内容](part7/7-3.md)
* [更多的动态加密：基于能力的加密](part7/7-3.md)
* [与经典 HTTP 网站相比（封建(feudal)安全等）](part7/7-4.md)

## 第 8 章：保持数据活跃：常驻(permanent) Web 上的可持续数据
* [简介](part8/README.md)
* [IPFS 集群](part8/8-1.md)
* [Filecoin](part8/8-2.md)

## 第 9 章：Merkle Trees 和 IPFS DAG
* [简介](part9/README.md)
* [将文件转换成哈希树](part8/8-1.md)
* [创建一个加密哈希](part8/8-2.md)
* [在 IPFS 中使用加密哈希来链接这些碎片（Merkle DAG）](part8/8-3.md)
* [探索使用哈希树来跟踪数据的软件类型](part8/8-4.md)

## 第 10 章：在 IPFS 上的动态内容
* [简介](part10/README.md)
* [免责声明：IPFS 上的动态内容是一项正在进行的工作](part10/10-1.md)
* [将数据添加到 DAG（本地）](part10/10-2.md)
* [将你的更改告诉 Peers ](part10/10-3.md)
* [使用哈希来从 IPFS 上获取他人的修改](part10/10-4.md)
* [使用发布订阅策略来传递有关修改的消息](part10/10-5.md)

## 第 11 章：分布式计算
* [简介](part11/README.md)

